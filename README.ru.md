...
- 🔘 **Кнопки переехали внутрь сообщения**: меню, мастера `/model` и `/think`, предложение обновления и «Работаю» с ⏹ стали богатыми сообщениями Telegram: каждая кнопка рядом на всю ширину, под ней пояснение, что она делает, экраны с заголовками, статус и таймеры таблицей, «закрыть/выключить» красные, «сохранить/включить» зелёные. Ива сама предлагает кнопки в ответе, когда вариантов два-четыре, и тап приходит ей как ваша реплика; скилл `rich-replies` описывает всю палитру (заголовки, таблицы, свёртки, сноски, формулы, картинки, слайды, карты и кнопки: выбор, ссылка, копирование). Нужен клиент Telegram от августа 2026; в группах кнопки в ответах не работают. [ADR-0015](docs/adr/0015-buttons-live-inside-the-message.md)

...
const EXPECTED_PRODUCTION_COUNT = 251;
const EXPECTED_INVENTORY_SHA256 =
  "793445e735dcfdccff643679d1c3eb9eee0b7664e2b270432875cbcade7c849e";
...
// The zoned-time move came last, one path: `agent/lib/zoned-time.ts`, the wall-clock
// arithmetic the schedule catch-up and the coming reminder surfaces share. Scoped coverage
// over `agent/lib/zoned-time.test.ts` and `agent/lib/schedule-migration.test.ts` reports it
...
// it.
// The owner rules came next, two paths: `agent/lib/owner-rules.ts`, the reader and the cap
// shared by the instruction source and the doctor, and `agent/instructions/30-owner-rules.ts`,
// the dynamic source eve loads alone. Scoped coverage over `scripts/owner-rules.test.ts`
// reports the reader, while the instruction source is loaded by eve alone - measured
// unreported, like `agent/instructions/20-core.ts` - so the blind spot stays 26.
// The rules-writing tool is gone (trim): `write_file` owns the slot, and the inventory
// counts 251 without the tool in it.
